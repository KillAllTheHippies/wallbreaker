...
export function RunsExplorer() {
  ...
  return <div className="v2-page v2-runs-explorer">
    <Panel title="Runs and logs" meta={`${runs.length} retained`}>
      <div className="v2-filterbar"><input aria-label="Search runs" value={runQuery} onChange={(event) => setRunQuery(event.target.value)} placeholder="Search runs" /><button type="button" className={`v2-button v2-button-small ${bookmarkedRunsOnly ? "active" : ""}`} aria-pressed={bookmarkedRunsOnly} onClick={() => setBookmarkedRunsOnly((current) => !current)}>★ Bookmarked</button><button type="button" className="v2-button v2-button-small" onClick={async () => { setMessage("Rebuilding index…"); try { await v2Api.rebuildHistory(); await loadRuns(); setMessage("History index rebuilt from canonical JSONL."); } catch (reason) { setMessage(reason instanceof Error ? reason.message : "Index rebuild failed"); } }}>Rebuild index</button></div>
      {bookmarks.error && <p className="v2-inline-status v2-error" role="alert">{bookmarks.error}</p>}
      <div className="v2-run-list">{visibleRuns.map((run) => {
        const target = { kind: "run" as const, key: run.name, label: run.name, run_name: run.name };
        return <div className={`v2-bookmark-row ${selectedRun === run.name ? "active" : ""}`} key={run.name}><button type="button" className="v2-row-select" onClick={() => setSelectedRun(run.name)}><strong>{run.name}</strong><span>{run.records || 0} events / {run.hits || 0} verdicts</span><small>{run.time || "Timestamp unavailable"}</small></button><BookmarkButton active={bookmarks.isBookmarked("run", run.name)} busy={bookmarks.busy === bookmarkId(target)} label={run.name} onClick={() => void bookmarks.toggle(target)} /></div>;
      })}</div>
    ...
    <Panel title={selectedRun || "Select a run"} meta={`${filtered.length} of ${events.length} events visible`}>
      <div className="v2-log-toolbar"><div className="v2-segmented" role="tablist" aria-label="Log view"><button type="button" className={view === "stream" ? "active" : ""} onClick={() => setView("stream")}>Stream</button><button type="button" className={view === "timeline" ? "active" : ""} onClick={() => setView("timeline")}>Timeline</button><button type="button" className={view === "raw" ? "active" : ""} onClick={() => setView("raw")}>Raw JSON</button></div><label className="v2-log-search"><span className="v2-sr-only">Search selected run</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search this run" /></label><button type="button" className={`v2-button v2-button-small ${bookmarkedEventsOnly ? "active" : ""}`} aria-pressed={bookmarkedEventsOnly} onClick={() => setBookmarkedEventsOnly((current) => !current)}>★ Bookmarked</button><FacetMenu label="Event types" values={typeValues} selected={types} onChange={setTypes} /><FacetMenu label="Actors" values={actorValues} selected={actors} onChange={setActors} />{verdictValues.length > 0 && <FacetMenu label="Verdicts" values={verdictValues} selected={verdicts} onChange={setVerdicts} />}<button type="button" className="v2-button v2-button-small" disabled={!filtered.length} onClick={() => download(`${selectedRun || "run"}-filtered.json`, JSON.stringify(filtered.map(eventData), null, 2))}>Export visible</button></div>
      ...
      {!loading && filtered.length > 0 && view === "stream" && <div className="v2-full-stream"><header><span>Chronological stream</span><button type="button" className="v2-button v2-button-small" onClick={() => navigator.clipboard.writeText(streamText)}>Copy stream</button></header>{filtered.map((event) => {
        ...
        return <article key={event.id} className={`v2-stream-${event.actor || "system"}`}><div><time>{event.timestamp}</time><strong>{event.actor || "system"}</strong><span>{event.event_type}</span>{event.technique && <span>{event.technique}</span>}{event.verdict && <VerdictBadge verdict={event.verdict} />}<BookmarkButton active={bookmarks.isBookmarked("event", key)} busy={bookmarks.busy === bookmarkId(target)} label={target.label} onClick={() => void bookmarks.toggle(target)} /></div><pre>{eventContent(event)}</pre></article>;
      })}</div>}
      {!loading && filtered.length > 0 && view === "timeline" && <div className="v2-history-browser v2-log-timeline"><div className="v2-history-events">{filtered.map((event) => {
        const key = eventBookmarkKey(event);
        const target = { kind: "event" as const, key, label: `${event.event_type} · ${event.timestamp}`, run_name: event.run_name, source_line: event.source_line };
        return <div className={`v2-bookmark-row ${selectedEvent?.id === event.id ? "active" : ""}`} key={event.id}><button type="button" className="v2-row-select" onClick={() => setSelectedEvent(event)}><span><strong>{event.event_type}</strong>{event.verdict && <VerdictBadge verdict={event.verdict} />}</span><span>{event.actor || "system"} / {event.technique || "unclassified"}</span><small>{event.timestamp}</small></button><BookmarkButton active={bookmarks.isBookmarked("event", key)} busy={bookmarks.busy === bookmarkId(target)} label={target.label} onClick={() => void bookmarks.toggle(target)} /></div>;
      })}</div><div className="v2-history-inspector">{selectedEvent ? <><dl className="v2-kv"><div><dt>Run</dt><dd>{selectedEvent.run_name}</dd></div><div><dt>Sequence</dt><dd>{selectedEvent.sequence}</dd></div><div><dt>Actor</dt><dd>{selectedEvent.actor || "--"}</dd></div><div><dt>Latency</dt><dd>{selectedEvent.latency_ms ?? "--"} ms</dd></div><div><dt>Tokens</dt><dd>{selectedEvent.input_tokens ?? 0} / {selectedEvent.output_tokens ?? 0}</dd></div><div><dt>Inference</dt><dd>{selectedEvent.inference_id || "--"}</dd></div></dl><JsonBlock value={eventData(selectedEvent)} /></> : <EmptyState title="Select an event" />}</div></div>}
